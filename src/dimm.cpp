                             ...
                             const uint8_t structLen, uint8_t* dataIn)
{
    ...
    {
        socketString = deviceLocator.substr(cpuPos + substrCpu.length(), 1);
    }
    ...
    {
        slotString = deviceLocator.substr(dimmPos + substrDimm.length() + 1);
    }

    /* slotString is extracted from substrDimm (DIMM_A) if slotString is single
     * alphabet like A, B , C.. then assign ASCII value of slotString to slot */
