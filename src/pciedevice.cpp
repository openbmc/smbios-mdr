                                      ...
                                      const std::string& slotPath)
{
    ...
    // Walk to the Nth PCIe slot entry, same logic as pcieslot.cpp
    for (uint8_t index = 0;
         index < deviceNum ||
    ...
    // GenerationInUse and GenerationSupported from slot type
    ...
    // MaxLanes from slot data bus width
    ...
    // LanesInUse unknown from SMBIOS Type 9 — use maxint (bmcweb omits this)
    PCIeDeviceIface::lanesInUse(std::numeric_limits<size_t>::max());

    // DeviceType unknown from SMBIOS Type 9
    PCIeDeviceIface::deviceType(PCIeDeviceType::Unknown);

#ifdef SLOT_DRIVE_PRESENCE
    PCIeDeviceItem::present(slotInfo->currUsage ==
                            static_cast<uint8_t>(Availability::InUse));
#else
    PCIeDeviceItem::present(true);
#endif

    // Create contained_by association linking this device to its PCIeSlot.
    // bmcweb's PCIeDevice handler queries pcieDevicePath + "/contained_by"
    // to resolve the associated PCIeSlot and populate the Slot sub-object.
    if (!slotPath.empty())
    {
