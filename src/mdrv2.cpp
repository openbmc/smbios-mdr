...
void MDRV2::systemInfoUpdate()
{
    ...
    for (unsigned int index = 0; index < *num; index++)
    {
        std::string path = getDimmObjectPath(smbiosInventoryPath, smbiosDir.dir[smbiosDirIndex].dataStorage, index);
...
std::string MDRV2::getDimmObjectPath(const std::string& smbiosInventoryPath, uint8_t* smbiosTableStorage, int dimmNum){
    // If some error happens, then abort out early with default object path
    std::string dimmObjectPath = smbiosInventoryPath + dimmSuffix + std::to_string(dimmNum);
    ...
    std::string dimmLocatorIndex = deviceLocator.substr(deviceLocator.find("DIMM") + 4);
