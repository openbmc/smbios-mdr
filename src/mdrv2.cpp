...
void MDRV2::upsertDimms(size_t dimm_count, const std::string& motherboardPath)
{
    ...
    for (size_t index = 0; index < dimm_count; index++)
    {
    ...
    if (deviceLocatorToIndex.size() == dimm_count)
    {
        size_t dimmsIndex = 0;
...
#endif
    for (unsigned int index = 0; index < dimm_count; index++)
    {
        std::string path =
