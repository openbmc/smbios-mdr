...
void MDRV2::upsertDimms(size_t dimmCount, const std::string& motherboardPath)
{
#ifdef USE_DEVICE_LOCATOR_AS_DIMM_OBJECT_PATH
    // Use sorted map to map deviceLocator to index
    std::map<std::string, size_t> deviceLocatorToIndex;

    for (size_t index = 0; index < dimmCount; index++)
    {
        std::optional<std::string> deviceLocator = getDeviceLocatorFromIndex(
            smbiosDir.dir[smbiosDirIndex].dataStorage, index);

        // If a dimm dosent have deviceLocator, we are using fallback index for
        // dimms.
        if (!deviceLocator)
        {
            break;
    ...
    // If every dimm has a locator, we can use device locator as object path
    if (deviceLocatorToIndex.size() == dimmCount)
    {
        size_t dimmsIndex = 0;

        for (const auto& [deviceLocator, index] : deviceLocatorToIndex)
        {
            std::string path = smbiosInventoryPath + deviceLocator;
            ...
            {
                dimms[dimmsIndex]->memoryInfoUpdate(
...
#endif
    for (unsigned int index = 0; index < dimmCount; index++)
    {
        std::string path =
...
void MDRV2::systemInfoUpdate()
{
    ...
    upsertDimms(*num, motherboardPath);
#endif
...
std::optional<std::string> MDRV2::getDeviceLocatorFromIndex(
