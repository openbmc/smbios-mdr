...
/**
 * Simple aggregate to define an external D-Bus property which needs to be set by
 * this application.
 */
struct CpuProperty
{
...
/**
 * List of properties we want to set. Items are added before sending the D-Bus
 * request, and are removed once a succesful D-Bus response is received.
 */
...
static void
    setDbusProperty(const std::shared_ptr<sdbusplus::asio::connection>& conn,
                    size_t cpu, std::list<CpuProperty>::iterator newProp);
...
static void
    tryReadSSpec(const std::shared_ptr<sdbusplus::asio::connection>& conn,
                 const std::shared_ptr<const CPUInfo>& cpuInfo)
{
    static std::optional<std::string> lastSSpec;
    ...
    logStream(cpuInfo->id) << "SSpec read status: " << static_cast<bool>(newSSpec) << "\n";
    if (newSSpec && newSSpec == lastSSpec)
    {
    ...
    // If this read worked (but comparison to last read failed), retry quickly
    // since it's either the first read or we're working around quick transient
    // PIROM failures during power flows. If this read failed, wait a longer
    // time and only retry a few times since the failure is not likely to
    // resolve (PIROM should always be readable).
    int retrySeconds;
    if (newSSpec)
    {
        retrySeconds = 1;
...
/**
 * Add a D-Bus property to the pending list, and attempt to set it by calling
 * `setDbusProperty`.
 *
 * @param[in,out]   conn        D-Bus connection.
 * @param[in]       cpu         CPU index to construct D-Bus object path.
                   ...
                   const std::string& propName, const std::string& propVal)
{
    ...
    setDbusProperty(conn, cpu, std::prev(propertiesToSet.end()));
}

/**
 * Set a D-Bus property which is already contained in the pending list. If the
 * set fails because the interface doesn't exist yet, setup a D-Bus match to
 * defer the property set until the interface is ready.
 *
 * @param[in,out]   conn    D-Bus connection.
 * @param[in]       cpu     CPU index.
 * @param[in]       newProp Property to set. Iterator must be from pending list
 *                          propertiesToSet.
 */
static void
    setDbusProperty(const std::shared_ptr<sdbusplus::asio::connection>& conn,
                    size_t cpu, std::list<CpuProperty>::iterator newProp)
{
    conn->async_method_call(
        [conn, cpu, newProp](const boost::system::error_code ec) {
            // See libsystemd/sd-bus/bus-error.c for mapping
            ...
            if (ec == boost::system::errc::host_unreachable ||
                ec == boost::system::errc::bad_address)
            {
            ...
            // Forget the property once it's been set successfully. This assumes
            // that any service hosting interfaces we modify will not remove and
            // add the interface again. We should also have a systemd dependency
            // on any services so that if they restart we will also restart.
            propertiesToSet.erase(newProp);
        },
 ...
 * pending properties targeting that object/interface.
 *
 * @param[in,out]   conn    D-Bus connection.
 * @param[in]       cpu     CPU index to construct D-Bus object path.
 */
    ...
    cpuUpdatedMatch.insert_or_assign(
        ...
        std::make_unique<sdbusplus::bus::match::match>(
            ...
            [conn, cpu](sdbusplus::message::message& msg) {
                ...
                // Go through all the pending property changes, and retry all of
                // them targeting this object/interface which was just added.
                for (auto iter = propertiesToSet.begin();
                     iter != propertiesToSet.end(); ++iter)
                {
                    if (iter->object == objectName &&
                        msgData.contains(iter->interface))
                    {
                        setDbusProperty(conn, cpu, iter);
                    }
                }
            }));
}
