                             ...
                             CPUList& cpus)
{
    ...
    for (auto& cpu : cpus)
    {
        ...
        if (peci_GetCPUID(cpu.peciAddress, &model, &stepping, &cc) !=
            PECI_CC_SUCCESS)
        {
            phosphor::logging::log<phosphor::logging::level::ERR>(
                "Cannot get CPUID!",
 ...
 */
static void getConfigValues(PECIManager& peciManager, int level,
 ...
 */
static void publishSSTConfigs(CPUList& cpus, sdbusplus::asio::connection& conn)
{
    for (auto& cpu : cpus)
    {
