                              ...
                              const std::string& interface)
{
    ...
    catch (const sdbusplus::exception_t& e)
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
            "Error in mapper method call ",
            phosphor::logging::entry("ERROR=%s", e.what()),
            phosphor::logging::entry("SERVICE=%s", objectPath.c_str()),
            phosphor::logging::entry("PATH=%s", interface.c_str()));
        return std::string{};
    }
...
static std::string getProperty(sdbusplus::bus::bus& bus,
                               const std::string& objectPath,
                               const std::string& interface,
                               const std::string& propertyName)
{
    ...
    try
    {
        ...
        std::variant<std::string> versionValue;
        reply.read(versionValue);
        version = std::get<std::string>(versionValue);
    }
    catch (const sdbusplus::exception_t& e)
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
...
std::string System::version(std::string value)
{
    ...
    if (getProperty(bus, biosActiveObjPath, biosVersionIntf, biosVersionProp) ==
        "NA")
    {
